...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _buildUserField(String label, String field, IconData icon) {
    ...
    if (field == 'dob') {
      try {
        if (value is Timestamp) {
          // Firestore Timestamp
          value = DateFormat.yMMMMd().format(value.toDate());
        } else if (value is String) {
          // ISO 8601 string
          final date = DateTime.parse(value);
          value = DateFormat.yMMMMd().format(date); // Example: January 22, 1990
        }
      } catch (e) {
        value = 'Invalid date';
      }
    ...
    return ListTile(
      ...
      subtitle: Text(value, style: const TextStyle(fontFamily: 'NexaBold')),
      trailing: IconButton(
